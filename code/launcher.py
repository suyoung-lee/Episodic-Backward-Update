...
def process_args(args, defaults, description):
    ...
    parser.add_argument('--deterministic', dest="deterministic",
                        action='store_false', default=defaults.DETERMINISTIC,
    ...
    parser.add_argument('--cudnn_deterministic', dest="cudnn_deterministic",
                        ...
                        help=('Whether to use deterministic backprop. ' +
                              '(default: %(default)s)'))
    parser.add_argument('--flickering-buffer', dest="flickering_buffer_size",
                        type=int, default=defaults.FLICKERING_BUFFER_SIZE,
                        help='anti flickering buffer size')
    parser.add_argument('--method', dest="method",
                        type=str, default=defaults.METHOD,
                        help='choose different learning algorithms')
    parser.add_argument('--transition-len', dest='transition_length',
                        type=int, default=4,
                        help='transition length in Optimality Tightening')
    parser.add_argument('--transition-range', dest='transition_range',
                        type=int, default=10,
                        help='transition range in Optimality Tightening Sampling')
    parser.add_argument('--penalty-method', dest='penalty_method',
    ...
    parser.add_argument('--weight-max', dest='weight_max',
                        type=float, default=0.8,
                        help='weight max for penalty method')
    parser.add_argument('--anneal-len', dest='annealing_len',
                        type=float, default=5000000,
                        help='annealing length of penalty method')
    parser.add_argument("--Seed", type=int, default=12)
    parser.add_argument("--beta", type=float, default=0.5) #constant diffusion factor
    parser.add_argument('--close2', dest="close2",
                        action='store_true', default=False,
                        help='choose close bounds')
    parser.add_argument('--late2', dest="late2",
                        action='store_true', default=False,
                        help='delay the penalty')
    ...
    if parameters.experiment_prefix is None:
        ...
        parameters.experiment_prefix = "Backward/code/Results/" + name + "/" + name + \
                                       time.strftime("_%m-%d-%H-%M-%S_", time.gmtime()) + parameters.method

    if parameters.double_dqn:
        parameters.experiment_prefix += '_(double)'

    parameters.experiment_prefix += '_' + str(parameters.learning_rate) + '_(ep' + str(parameters.epochs) + ')'
...
def launch(args, defaults, description):
    ...
    if parameters.method == 'ot':
        ...
        agent = ale_agents.OptimalityTightening(network,
                                                ...
                                                parameters.transition_range,
                                                parameters.penalty_method,
                                                parameters.weight_min,
                                                parameters.weight_max,
                                                parameters.annealing_len,
                                                parameters.beta,
                                                parameters.two_train,
                                                parameters.late2,
                                                parameters.close2,
                                                parameters.verbose,
    ...
    experiment = ale_experiment.ALEExperiment(ale, agent,
                                              ...
                                              rng,
                                              parameters.flickering_buffer_size)
